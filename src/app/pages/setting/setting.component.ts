...
export class SettingComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    if (
      this.currentUser.playlist_sets &&
      this.currentUser.playlist_sets.length > 0
    )
      this.currentUser.playlist_sets.forEach((set) => {
        this.playlistSets.push(
          this.formBuilder.control(set, Validators.required),
        );
      });
    else
  ...
  async onSubmit() {
    ...
    await this.userService.update(this.transform(), [
      'name',
      'city',
      'picture',
      'playlist_sets',
