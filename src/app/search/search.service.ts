...
export class SearchService {
  ...
  async query(humanQuery: string, from: number = 0, size: number = 20) {
    return this.queryTrack(
      transformHumanQueryToElasticQuery(
        humanQuery,
        (await this.authen.auth.currentUser).uid,
      ),
      from,
      size,
    );
  ...
  async search(
    ...
    if (options && Object.keys(options).length > 0) {
      ...
      if (terms.length > 0) params.push(q);
      if (options.tempo) {
