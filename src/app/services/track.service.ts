...
export class TrackService {
  ...
  toggle(trackID: string, spotifyTrack?: SpotifyTrackItem) {
    ...
    Object.keys(this.playedStatusTrack)
      .filter((id) => id != trackID)
      .forEach((id) => (this.playedStatusTrack[id] = false));
    if (!this.playedStatusTrack[trackID])
      this.playedStatusTrack[trackID] = true;
    else this.playedStatusTrack[trackID] = false;
    if (this.playedStatusTrack[trackID])
  ...
  async getCommunityPlaylists() {
    let snap = await this.firestore
      .collection('community_playlist', (ref) => ref.orderBy('created', 'desc'))
      .get()
      .toPromise();
    return Promise.all(
      snap.docs
        ...
        .map(async (item) => ({
          ...item,
          by: await this.userService.get(item.by),
        })),
    );
  }

  async getStaffPicks() {
    return fetch(`${environment.api_url}/staffPicks`)
      .then((res) => res.json())
      .then(
        (res) =>
          res as Array<{
