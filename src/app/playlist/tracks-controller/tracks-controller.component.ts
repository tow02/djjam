...
export class TracksControllerComponent implements OnChanges {
  ...
  ngOnChanges(): void {
    if (this.playlistEvent.status === 'done') {
      ...
      this.trackSource = this.playlistEvent.playlist.tracks.items
        ...
        .map((item) => {
          ...
          //update bpm;
          trackElement.bpm = this.spotifyService._processSpotifyTrackToBpm(
            item,
            this.playlistEvent.djjamTracks,
            this.playlistEvent.audioFeatures,
          );
          trackElement.tags = this.spotifyService
            ._processTagsToArrayTags(
              this.spotifyService._processSpotifyTrackToTags(
                item,
                this.playlistEvent.djjamTracks,
                this.playlistEvent.audioFeatures,
              ),
          ...
          trackElement.personal_tags = this.spotifyService
            ._processTagsToArrayTags(obj)
            .join(',');
          return trackElement;
        });
