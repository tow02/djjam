...
export class ArtistsCharacterComponent implements OnChanges {
  ...
  async ngOnChanges() {
    if(this.playlistEvent.status == "done"){
      this.artistInfomation = this.spotifyService.getArtistsInfomation(this.playlistEvent.playlist);
      const ids = Object.keys(this.artistInfomation).map(item => ({id:item, count:this.artistInfomation[item].count })).sort( (a,b) => {
        if(a.count > b.count)
          return -1
        else
          return 1;
