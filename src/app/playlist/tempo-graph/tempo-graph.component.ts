...
export class TempoGraphComponent implements OnChanges {
  ...
  public barChartLabels: Label[] = environment.analytics.rangeNames.map(
    (item) => `${item.name} (${item.bpmMin} - ${item.bpmMax})`,
  );
  ...
  getBpmRanges() {
    this.bpmTracks = this.spotifyService.getTrackBpms(
    ...
    //get bpm range
    return this.bpmTracks
      .reduce(
        (prev, current) => {
          let plus = environment.analytics.rangeNames.map((item) => {
            if (current >= item.bpmMin && current < item.bpmMax) return 1;
            else return 0;
          });
  ...
  ngOnChanges(): void {
    ...
    if (this.playlistEvent.status == 'done') {
      ...
      this.barChartData = [
        { data: this.getBpmRanges(), label: ` % `, backgroundColor: '#ffc218' },
      ];
    }
  }
}
