...
function _artistSearch(original_tracks: Array<any>, q: string) {
  return original_tracks.filter((track) => {
    // return (track.name as string).toLowerCase().indexOf(q.toLowerCase()) >= 0
    return (
      (track.artists as Array<any>).filter((artist) => {
        return (
          (artist.name as string).toLowerCase().indexOf(q.toLowerCase()) >= 0
        );
      }).length > 0
    );
  });
...
function _getBpmRange(q: string) {
  ...
  if (!q.match(double_reg)) return null;
...
function _bpmSearch(original_tracks: Array<any>, q: string) {
  ...
  return original_tracks.filter((track) => {
    if (!track.bpm || !realData) return false;
...
function _tagSearch(original_tracks: Array<any>, q: string) {
  return original_tracks.filter((track) => {
    ...
    if (!track.tags[q.toLocaleLowerCase()]) return false;
...
export function batchUnTag(tracks: Array<any>, tag: string) {
  tracks.forEach((item) => {
    if (item.tags && item.tags[tag]) delete item.tags[tag];
  });
  return tracks;
}
...
function _getTagsQuery(q: string, isExcluded: boolean = false) {
  let command_regex;
  if (isExcluded) command_regex = /exclude\s"[\w\d\s,]+"/;
  else command_regex = /tag\s"[\w\d\s,]+"/;
  if (!q.match(command_regex)) return [];
  let _tags: string = q.match(command_regex)[0];
  let reg2 = _tags.match(/"[\w\d\s,]+"/)[0];
  console.log(reg2);
  console.log('after_tag');
  let tags = reg2.match(/[^,^"]+/g).filter((text) => {
    return text.length > 0;
  });
  return tags;
}

function specificTagsSearch(
  original_tracks: Array<any>,
  q,
  ...
  if (isExcluded)
    ...
    stupid_tracks = result_tags.reduce((prevTerm, currentTerm, index) => {
      ...
      (prevTerm as Array<any>).forEach((track) => {
        ...
        if (intersectIndex >= 0) tracks.push(track);
      });
      return tracks;
    });
  }
...
function excludeCommand(q: string) {
  ...
  command_regexs.forEach((regex) => {
    if (q.match(regex)) filter_q = filter_q.replace(q.match(regex)[0], '');
  });
  return filter_q;
}

export function searchTrack(original_tracks: Array<any>, q: string) {
  if (!q) return original_tracks;
  //
  //this.q = q;
  ...
  terms.forEach((term, index) => {
    ...
    _result.forEach((result) => {
      ...
      _terms[index] = [];
      for (let key in _merge) _terms[index].push(_merge[key]);
    });
  });
  //add command search
  if (bpm_result.length > 0) _terms.push(bpm_result);
  if (tags_result.length > 0) _terms.push(tags_result);
...
export function getBpmDataRange(tracks: Array<any>) {
  ...
  environment.analytics.rangeNames.forEach((item) => {
    ...
    bpm_data_range.push({
      name: item.name,
      percent: Math.floor((count / tracks.length) * 10000) / 100,
      min: item.bpmMin,
      max: item.bpmMax,
    });
  });
...
export function getTempoMetaData(tracks: Array<any>) {
  let obj = {
    ...
    duration_average: 0,
    bpm_average: 0,
    eight_count_per_song_average: 0,
  };
  let bpm_sum = 0;
  let duration_sum = 0;
  tracks.forEach((t) => {
  ...
  obj.eight_count_per_song_average =
    ((obj.duration_average / 60000) * obj.bpm_average) / 8;
  return obj;
}
...
export function transformHumanQueryToElasticQuery(
  ...
  if (mustNotHaveTags && mustNotHaveTags.length > 0) {
    ...
    if ((mustHaveTags && mustHaveTags.length > 0) || bpmQ) q += ' AND ';
    q += ` NOT (${exq})`;
  }
  // normal search
  let normal_q = '';
  ...
  terms
    .filter((item) => item != '')
    .forEach((term, index) => {
      if (index > 0) normal_q += ' AND ';
      normal_q += `(artists:${term} OR name:${term} OR tags:${term} OR (personal_tags:${term}^100 AND personal_tags:${user_id_token} ))`;
    });

  if (
    (mustNotHaveTags && mustNotHaveTags.length > 0) ||
    (mustHaveTags && mustHaveTags.length > 0) ||
    bpmQ
  ) {
    if (normal_q && normal_q.length > 0) q += ` AND ${normal_q}`;
  } else q = normal_q;
